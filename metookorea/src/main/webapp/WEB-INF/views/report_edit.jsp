...
<body class="body-wrapper">
			...
			<div class="col-md-8 col-sm-8 col-md-offset-2 col-sm-offset-2">
				<div class="login-form">
					...
					<c:forEach var="listValue" items="${reportDetail}">
						...
						<form role="form" id="editForm"   method="post" enctype="multipart/form-data"   action="${pageContext.request.contextPath}/report_edit_process">
							<div class="row">
								<div class="col-md-8">
									<div class="form-group">
										<label for="exampleInputEmail1">* 가해자 이름</label> <input type="text" class="form-control" maxlength="40" name="A_name" id="A_name" value="${listValue.m2_report_Aname }">
									</div>
									<c:if test="${listValue.m2_report_Aispublic eq 'on'}">
										<c:set var="ispublic" value="checked"></c:set>
									</c:if>
									<div class="checkbox">
										<label> <input type="checkbox" id="A_ispublic" name="A_ispublic" ${ispublic}> 가해자가 공인입니까?
										</label>
									</div>
							...
							<div class="form-group">
								<label for="exampleInputEmail1"> 가해자 직업 또는 소속</label> <input type="text" class="form-control" maxlength="50" name="A_job" id="A_job" value="${listValue.m2_report_Ajob }"
									placeholder="가해자의 직업이나 소속을 적어주세요"> <label> </label>
							</div>
							<div class="form-group">
								<label for="exampleInputEmail1">* 신고자 닉네임</label> <input type="text" class="form-control" maxlength="50" name="R_name" id="R_name" value="${listValue.m2_report_Rname }"
									placeholder=" 신고자 닉네임을
									적어주세요. 본명을 밝히고 싶으시면, 본명도괜챃습니다."> <label> </label>

							</div>
							<div class="form-group">
								<label for="exampleInputEmail1"> 피해 당시 연령</label> <input type="text" class="form-control" maxlength="50" name="R_age" id="R_age" value="${listValue.m2_report_Rage }"
									placeholder="피해 당시의 연령을 적어주세요"> <label> </label>
							</div>
							<div class="form-group">
								<label for="exampleInputEmail1">* 제보 제목</label> <input type="text" class="form-control" maxlength="100" name="report_title" id="report_title" value="${listValue.m2_report_title }"
									placeholder="제보 제목"> <label> </label>
							</div>
							<div class="form-group">
								<label for="exampleInputEmail1">* 제보내용</label>
								<div style="text-align: center;">
									<textarea rows="10" style="width: 100%" name="report_content" id="report_content">${listValue.m2_report_content }</textarea>
								</div>
							...
							<div class="img_wrap">
								<img src="${pageContext.request.contextPath}/${listValue.m2_report_link }" id="preview_img" style="margin: 0 auto;" class="img-responsive">
							</div>

							<div style="text-align: center; margin: 8px">
								<a type="submit" onclick="report_delete()" class="btn btn-theme-bg" style="width: 80%">제보 삭제하기</a>
							</div>

							<div style="text-align: center; margin: 8px">
								<a type="submit" onclick="report_submit()" class="btn btn-theme-bg" style="width: 80%">제보 수정하기</a>
							</div>

							<sec:authorize access="hasRole('ROLE_ADMIN')">
								<div style="text-align: center; margin: 8px">
									<a type="submit" onclick="report_submit_article()" class="btn btn-theme-bg" style="width: 80%">제보 수정(기사정보인 미투리스트 목록일때만!!)</a>
								</div>
							</sec:authorize>
	...
	<script type="text/javascript">
		...
		function report_delete() {
			var result = confirm('해당 제보를 정말 삭제하시겠습니까?');
			if (result) {
				//값 셋팅
				//report_rcdno
				...
				// ajax 호출
				$.ajax({
					url : "${pageContext.request.contextPath}/report_delete_process",
					...
					success : function(data) {
						if (data == "success") {
							alert("성공적으로 삭제했습니다");
							location.href = "${pageContext.request.contextPath}/me_report_list";
		...
		function report_submit() {
			...
			if (checkingInputOk) {
				...
				$('#editForm').ajaxForm({
					url : "${pageContext.request.contextPath}/report_edit_process",
					...
					success : function(data) {
						...
						if (aJaxResult == "success") {
							alert('제보 내용을 성공적으로 수정했습니다');
							var m2_report_rcdno = data["report_rcdno"];
		...
		function report_submit_article() {
			var checkingInputOk = inputTagsCheckingOk();
			if (checkingInputOk) {
				//에디터 내용 가져옴
				var report_content_ckeditor = CKEDITOR.instances.report_content.getData();
