...
public class MemberDao {
	...
	public int insertNewUserIntoDatabaseByObj(userDTO targetObj) {
		System.out.println("this is insertNewUserIntoDatabaseByObj() Started");
		IDao dao = sqlsession.getMapper(IDao.class);
		int insertByObj_result = dao.insertNewUserIntoDatabaseByObj(targetObj);
		return insertByObj_result;
	}
	
	public int insertUserRoleIntoDatabaseByMap(Map targetMap) {
		IDao dao = sqlsession.getMapper(IDao.class);
		int insertByMap_result = dao.insertUserRoleIntoDatabaseByMap(targetMap);
		return insertByMap_result;
	}
	
	public userDTO passwordCheck(Map targetMap) {
		IDao dao = sqlsession.getMapper(IDao.class);
		userDTO selectedUser = dao.passwordCheck(targetMap);
		return selectedUser;
